...
private class LocalFileIntents(private val context: Context, private val task: DownloadTask, isMedia: Boolean) {
    private val mimeType = if (isMedia) "video/*" else "*/*"
    ...
    fun openContainingFolder() = openContainingFolder(context, task.destinationPath)
}
...
internal fun LocalTransferRow(
    ...
    val intents = remember(context, task, isMedia) { LocalFileIntents(context, task, isMedia) }
    ...
    val actions = buildList {
        ...
        when (task.status) {
            DownloadStatus.COMPLETED -> {
                ...
                add(SheetAction(Icons.Outlined.Share, "分享", intents::share))
            }
...
private fun downloadUri(context: Context, path: String): Uri? {
...
private fun openContainingFolder(context: Context, path: String) {
