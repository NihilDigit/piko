...
- 🚀 **8 连接并发流式播放**：参考 Animeko 播放架构，结合 Media3 (ExoPlayer) 与 `PikPakStreamReader` 实现 8 线程并行取流、LRU 分块预取与高命中缓存，实现秒开拖拽不卡顿。
- ✂️ **视频指定段落裁剪下载**：视频文件提供专属段落下载功能，支持开始/结束双时间点实时帧画面预览、微步微调芯片（±1s / ±10s）与进度滑块，底层走 SDK 8 连接切片范围下载。
- ⚡ **8 连接并发多任务下载**：内置高性能下载引擎，支持断点续传、并发控制与通知栏实时速率更新。
